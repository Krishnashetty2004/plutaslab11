...
interface InfiniteGalleryProps {
	...
	/** Callback for scroll velocity changes */
...
function GalleryScene({
	...
	const handleWheel = useCallback(
		(event: WheelEvent) => {
			// Only prevent default if scrolling within the canvas bounds
			...
			if (isWithinBounds) {
				...
				setScrollVelocity((prev) => {
					const newVelocity = prev + event.deltaY * 0.01 * speed;
					if (onScrollVelocityChange) {
						onScrollVelocityChange(newVelocity);
					}
					return newVelocity;
				});
				setAutoPlay(false);
	...
	const handleKeyDown = useCallback(
		(event: KeyboardEvent) => {
			if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') {
				setScrollVelocity((prev) => {
					const newVelocity = prev - 2 * speed;
					if (onScrollVelocityChange) {
						onScrollVelocityChange(newVelocity);
					}
					return newVelocity;
	...
	useFrame((state, delta) => {
		...
		setScrollVelocity((prev) => {
			...
			// Call the callback with the new velocity
			if (onScrollVelocityChange) {
				onScrollVelocityChange(newVelocity);
			}
			return newVelocity;
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-gray-100 p-4">
			...
			<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4 max-h-96 overflow-y-auto">
		...
		<div className={className} style={style}>
			...
			>
				<GalleryScene
					...
					fadeSettings={fadeSettings}
					blurSettings={blurSettings}
					onScrollVelocityChange={onScrollVelocityChange}
				/>
			</Canvas>
		</div>
	);
